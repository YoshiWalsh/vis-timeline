...
function performStacking(items, margins, compareTimes, shouldStack, shouldOthersStack, getInitialHeight, shouldBail) {
  ...
  for(const item of items) {
    if(shouldStack(item)) {
      ...
      if(shouldOthersStack(item)) {
        ...
        if(previousStart !== null && itemEnd < previousEnd - EPSILON) {
          insertionIndex = 0;
        }
        previousEnd = itemEnd;

        insertionIndex = findIndexFrom(itemsAlreadyPositioned, i => getItemEnd(i) - EPSILON > itemEnd, insertionIndex);
  ...
  while(itemsToPosition.length > 0) {
    ...
    if(shouldOthersStack(item)) {
      ...
      insertionIndex = binarySearch(itemsAlreadyPositioned, (target, i) => target - getItemEnd(i), itemEnd);
