...
function performStacking(items, margins, compareTimes, shouldStack, shouldOthersStack, getInitialHeight, shouldBail) {
  ...
  for(const item of items) {
    if(shouldStack(item)) {
      ...
      if(shouldOthersStack(item)) {
        ...
        insertionIndex = findIndexFrom(itemsAlreadyPositioned, i => getItemStart(i) - EPSILON > itemStart, 0);
  ...
  while(itemsToPosition.length > 0) {
    ...
    horizontalOverlapEndIndex = findIndexFrom(itemsAlreadyPositioned, i => itemEnd < getItemStart(i) - EPSILON, horizontalOverlapStartIndex);
    ...
    if(shouldOthersStack(item)) {
      ...
      insertionIndex = binarySearch(itemsAlreadyPositioned, (target, i) => target - getItemStart(i), itemStart);
...
function binarySearch(arr, comparison, target) {
  let minIndex = 0;
  ...
  while(minIndex < maxIndex) {
    ...
    const comparisonResult = comparison(target, arr[mid]);

    if(Math.abs(comparisonResult) < EPSILON) {
      return mid;
    }
