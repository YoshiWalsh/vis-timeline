  ...
  for (i = 0; i < itemsToPosition.length; i++) {
    ...
    if (shouldStack(item)) {
      ...
      do {
        ...
        for (let j = 0, jj = horizontallyCollidingItems.length; j < jj; j++) {
          const other = horizontallyCollidingItems[j];
          ...
          if (other.top !== null && other !== item && itemsCollideVertically) {
            collidingItem = other;
            break;
          }
        }
        ...
        if (collidingItem != null) {
          ...
          item.top = collidingItem.top + collidingItem.height + margins.vertical;
        }
